...
class DataTable(object):
        ...
        if isinstance(value, tuple):
            ...
            if (len(value) not in [2, 3] or
                ...
                raise DataTableException("Wrong format for value and formatting - %s." %
                                         six.text_type(value))
            if not isinstance(value[1], (six.text_type, six.binary_type, type(None))):
    ...
    def TableDescriptionParser(table_description, depth=0):
        ...
        if isinstance(table_description, (six.binary_type, tuple)):
        ...
        if (len(table_description) != 1 or
                (isinstance(list(six.iterkeys(table_description))[0], (six.text_type, six.binary_type)) and
                     isinstance(six.itervalues(table_description)[0], tuple) and
                         len(six.itervalues(table_description)[0]) < 4)):
        ...
        parsed_col = DataTable.ColumnTypeParser(list(six.iterkeys(table_description))[0])
        parsed_col["depth"] = depth
        parsed_col["container"] = "dict"
        return ([parsed_col] +
    ...
    def _PreparedData(self, order_by=()):
        ...
        proper_sort_keys = []
        if isinstance(order_by, (six.text_type, six.binary_type)) or (
        ...
        def SortCmpFunc(row1, row2):
            ...
            def py3cmp(a, b):
                # cmp() was removed entirely in py3
                return (a > b) - (a < b)

            for key, asc_mult in proper_sort_keys:
                cmp_result = asc_mult * py3cmp(row1[0].get(key), row2[0].get(key))
                if cmp_result:
                    return cmp_result
            return 0
